...
TransactionBuilder.prototype.sign = function (index, keyPair, redeemScript, hashType) {
  ...
  var valid = input.pubKeys.some(function (pubKey, i) {
    ...
    console.log("myhash="+(bcrypto.hash256(input.redeemScript)).toString('hex'));
    console.log("sighash=" + signatureHash.toString('hex'));
    input.signatures[i] = keyPair.sign(signatureHash)
    console.log("signatur1=" + input.signatures[i].toScriptSignature(1).toString('hex'));
    console.log("signatur1=" + input.signatures[i].toScriptSignature(1).length);
